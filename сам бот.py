		...
		if room != "":
			...
			for i in f:
				if fio in i.split(";")[1]:
					...
					break
			if clone:
				bot.send_message(message.from_user.id, "Вы уже регистрировались,ваш ID:" + i.split(";")[0])
			else:
	...
	for _ in text:
		if fio in text:
			...
			for i in stroka:
				if fio in i:
					info = i.split(";")
					bot.send_message(message.from_user.id, "Ваш ID: " + info[0])
			break
		else:
...
def find_dudes(message):
	...
	for i in f:
		if info == i.split(";")[1] or info == i.split(";")[0]:
			if int(i.split(";")[3]) == 1:
				room = "Сайт для Драйв.Ру"
			elif int(i.split(";")[3]) == 2:
				room = "Сайт для Lacoste"
			elif int(i.split(";")[3]) == 3:
				room = "Редизайн для ГосУслуг"
			elif int(i.split(";")[3]) == 4:
				room = "Сайт для Газпрома"
			bot.send_message(message.from_user.id, "Ваш Человек: " + i.split(";")[1] + "\nЕго ID: " + i.split(";")[0] + "\nЕго команда и должность: " + room + "," + i.split(";")[2])
		else:
...
def menu(message):
	if message.chat.type == 'private':
		if message.text == "Регистрация":
			bot.send_message(message.chat.id, "Хорошо, тогда введите ваше ФИО, должность и ID команды, к которой хотите \
